...
`NSL-KDD`
- `DoS` - service-disruption attacks that exhaust or block availability
- `Probe` - scanning and reconnaissance to map hosts or services
- `R2L` - remote-to-local intrusions that try to gain local access
- `U2R` - user-to-root privilege escalation after initial access

`CIC-UNSW-NB15`
- `Fuzzers` - malformed or random-input attacks to trigger faults
- `Analysis` - reconnaissance or testing traffic used to study a target
- `Backdoor` - hidden-access mechanisms that bypass normal authentication
- `DoS` - availability attacks that flood or stall a service
- `Exploits` - attacks that abuse known software flaws
- `Generic` - broad cryptographic or protocol abuse traffic in the UNSW taxonomy
- `Reconnaissance` - scanning and target-enumeration traffic
- `Shellcode` - payload delivery intended to execute attacker code
...
- `DoS Hulk` - high-rate HTTP flooding denial-of-service traffic
- `DoS GoldenEye` - HTTP-layer denial-of-service with connection abuse
- `DoS slowloris` - slow connection-holding denial-of-service traffic
- `DoS Slowhttptest` - slow HTTP request denial-of-service traffic
- `Heartbleed` - exploitation traffic targeting the OpenSSL Heartbleed flaw
- `Web Brute Force` - repeated login or credential-guessing web attacks
- `Web XSS` - cross-site scripting attack traffic
- `Web Sql Injection` - SQL injection attack traffic
- `Infiltration` - unauthorized foothold or internal compromise traffic

`CSE-CIC-IDS2018` measured snapshot
- `DDOS attack-HOIC` - high-volume HTTP flood traffic from HOIC
- `DDoS attacks-LOIC-HTTP` - HTTP flood traffic generated by LOIC
- `DoS attacks-Hulk` - single-source HTTP flooding denial-of-service traffic
- `Bot` - botnet or remotely controlled infected-host traffic
- `FTP-BruteForce` - brute-force login attempts against FTP services
- `SSH-Bruteforce` - brute-force login attempts against SSH services
- `Infilteration` - infiltration or post-compromise internal traffic
- `DoS attacks-SlowHTTPTest` - slow HTTP denial-of-service traffic
- `DoS attacks-GoldenEye` - GoldenEye-style application-layer DoS traffic
- `DoS attacks-Slowloris` - slow connection exhaustion denial-of-service traffic
- `DDOS attack-LOIC-UDP` - UDP flooding traffic generated by LOIC
- `Brute Force -Web` - web login brute-force traffic
- `Brute Force -XSS` - web attack traffic associated with XSS probes/exploitation
- `SQL Injection` - SQL injection web attack traffic

`CIC-DDoS2019` measured snapshot
- `TFTP` - flooding traffic abusing the TFTP service
- `Syn` - TCP SYN flooding denial-of-service traffic
- `MSSQL` - flooding traffic targeting Microsoft SQL services
- `DrDoS_SNMP` - reflected or amplified SNMP-based DDoS traffic
- `DrDoS_DNS` - reflected or amplified DNS-based DDoS traffic
- `DrDoS_MSSQL` - reflected or amplified MSSQL-based DDoS traffic
- `DrDoS_NetBIOS` - reflected or amplified NetBIOS-based DDoS traffic
- `UDP` - direct UDP flooding traffic
- `NetBIOS` - direct floods targeting NetBIOS services
- `DrDoS_UDP` - reflected or amplified UDP-based DDoS traffic
- `DrDoS_SSDP` - reflected or amplified SSDP-based DDoS traffic
- `DrDoS_LDAP` - reflected or amplified LDAP-based DDoS traffic
- `LDAP` - direct flooding traffic targeting LDAP services
- `DrDoS_NTP` - reflected or amplified NTP-based DDoS traffic
- `UDP-lag` - lag-inducing UDP flooding traffic
- `Portmap` - flooding traffic abusing the Portmap service
- `UDPLag` - alternate UDP-lag label variant in the source data
- `WebDDoS` - web-application-layer distributed denial-of-service traffic
